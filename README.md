...
- **병인 분류**(test 80명): E2·E3 는 PASS 지만 **주 지표 E1(LAA-vs-CE)은 판정 불가** → 헌장의 종합 판정 규칙에 따라 **병인 파트 전체는 [판정 불가]** 로 보고합니다.
...
> — 즉 이 증분은 test 80명에서 잡음과 구분되지 않는다. PASS 를 "효과가 입증됐다"로 읽지 않는다.
...
역전 폭 자체는 test 표본에서 잡음과 구분되지 않지만(Δ −0.010 [−0.195, +0.170]), **dev 에서 확인한 우위가 test 에서 재현되지 않았다**는 사실은 그대로 남는다.
